/**
 * Calls chart - accepts array of { label, value } or { period, calls } etc.
 */
...
function getValue(item) {
  const n = item?.value ?? item?.calls ?? item?.count ?? item?.total ?? 0;
  return Number(n) || 0;
}

export default function CallsChart({ data, loading, title = 'Calls' }) {
  ...
  if (items.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-700 mb-4">{title}</h3>
        <div className="h-48 flex items-center justify-center text-slate-400 text-sm">No calls data yet.</div>
      </div>
    );
  }
